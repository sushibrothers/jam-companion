...
def render_dynamic_fretboard(audio_base64, audio_type, segments, scale_notes):
    ...
    <body>
      <div class="card">
        ...
        <div style="margin-top: 12px;">
          <div style="font-size: 12px; font-weight: bold; color: #94a3b8; margin-bottom: 4px;">Línea de tiempo interactiva (Haz clic en cualquier acorde para saltar a ese segundo):</div>
      ...
      <script>
        ...
        function drawFretboard(rootNote, arpeggioNotes) {{
          ...
          // Dots
          const dotPositions = "3,5,7,9".split(",").map(Number);
          dotPositions.forEach(function(f) {{
          ...
          // Strings
          tuning.forEach(function(openNote, sIdx) {{
          ...
          tuning.forEach(function(openNote, sIdx) {{
            ...
            for (let f = 0; f <= numFrets; f++) {{
              ...
              }} else if (arpeggioNotes && arpeggioNotes.indexOf(note) !== -1) {{
              ...
              }} else if (scaleNotes && scaleNotes.indexOf(note) !== -1) {{
        ...
        function onTick() {{
          ...
          if (activeIdx !== -1 && activeIdx !== lastActiveIdx) {{
            ...
            let badgesHtml = '';
            seg.arpeggio.forEach(function(n) {{
        ...
        if (segments.length > 0) {{
          ...
          let initialBadges = '';
          segments[0].arpeggio.forEach(function(n) {{
        ...
        audio.addEventListener('play', function() {{ requestAnimationFrame(onTick); }});
...
st.write("Sube tu canción para analizarla y ver el mástil sincronizado con la reproducción en tiempo real.")
...
if st.session_state.get('analysis_done', False):
    ...
    st.divider()
    st.subheader("▶️ Mástil Dinámico Sincronizado en Tiempo Real")
    player_html = render_dynamic_fretboard(
        st.session_state['audio_b64'],
        st.session_state['audio_mime'],
        st.session_state['segments'],
        st.session_state['scale_notes']
    )
    st.components.v1.html(player_html, height=580)
